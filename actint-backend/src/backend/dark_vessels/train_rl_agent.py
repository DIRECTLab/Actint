...
def train_rf_baseline(train_df: pd.DataFrame, eval_df: pd.DataFrame) -> dict:
    ...
    # This adds stuff to the clf ActivityIntelligenceClassifier training model for it to train on.
    clf.fit(train_df)
    ...
    # This gets some reports/information from the results of training.
...
def main(visualise: bool = False):
    ...
    # This simulates a bunch of AIS data
    ...
    # The normalize method where you can give it new data and it will determine the z-score (how many standard deviations it is away rom the mean)
    # There is also a fit method which allows you to send 2-d data into the normalizer and it will add all of them one by one
    ...
    # This takes a bunch of simulated AIS data and uses it to train a random forest model to predict the activity and type of all the vessels. 
    # It also predicts the probability of a ship going dark, doing illegal fishing, or doing a rendezvous. 
    # It then evaluates the model on the eval_df and returns a dictionary of metrics that describe how well the model did. 
    # This is used as a baseline to compare the RL agent against.
    ...
    if args.eval_only:
        ...
        # This is the thing that actually trains the ML network. 
        # It trains to recognise the actions the vessels are taking.
