...
from backend.mcp_servers.ais.helpers.vessel_query import get_vessel_name, get_vessel_position_history
from backend.mcp_servers.ais.helpers.vessel_query import get_vessel_position_history
...
def ship_following(mmsi1, mmsi2):
    ...
    THRESHOLD_TIME = 60 * 60 * 1000 * 1  # 1 hour in milliseconds
    THRESHOLD_DISTANCE = 5.0 #Latitude/longitude degrees for now

    positions1 = get_vessel_position_history(mmsi1)
    positions2 = get_vessel_position_history(mmsi2)
    ...
    string = f"Vessel {mmsi2_name} went to the same area as {mmsi1_name} within {THRESHOLD_TIME/(1000*60*60)} hours of when {mmsi1_name} was there {sum(in_previous_area)}/{len(in_previous_area)} times."
    return string