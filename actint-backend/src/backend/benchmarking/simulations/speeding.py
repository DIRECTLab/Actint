...
try:
	from backend.benchmarking.simulations.base_simulation import BaseSimulation
...
class SpeedingSimulation(BaseSimulation):
	...
	def inject_anomalies(
		...
		replacement_mmsis: list[int] | None,
		...
		try:
			if replacement_mmsis:
				source_candidates = self.get_source_vessels_by_mmsis(
					conn,
					target_mmsis=replacement_mmsis,
			...
			dataset_start_time, _ = self.get_dataset_time_bounds(conn)
			...
			for i in range(ships_to_replace):
				...
				conn.execute(
					...
					(
						...
						self.to_sqlite_timestamp(start_time),
						self.to_sqlite_timestamp(start_time + timedelta(seconds=track_duration_seconds)),
					),
				)
	...
	def write_benchmark_config(self, config_path: Path, expected_mmsis: list[int]) -> None:
		...
		prompt_template = (
			"Use available MCP tools to identify ships with sudden spikes in speed in the AIS database.\\n"
	...
	def run_from_args(self, args: argparse.Namespace) -> int:
		...
		replacement_mmsis = self.resolve_replacement_mmsis(
			db_path=output_db,
			min_source_positions=args.min_source_positions,
			ships_to_replace=args.ships_to_replace,
			replace_mmsis=args.replace_mmsis,
		)
		ships_to_replace = (
			len(replacement_mmsis)
			if replacement_mmsis
			else self.validate_ships_to_replace(
				db_path=output_db,
				min_source_positions=args.min_source_positions,
				ships_to_replace=args.ships_to_replace,
			)
		...
		anomalies = self.inject_anomalies(
			...
			replacement_mmsis=replacement_mmsis,
