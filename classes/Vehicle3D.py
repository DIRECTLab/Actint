...
def normalize(v: Position.Position3D) -> Position.Position3D:
    ...
    if norm > 1e-9:
        return Position.Position3D(*[float(x) for x in v_vec / norm])
    return Position.Position3D(0.0, 0.0, 0.0)

def truncate(v: Position.Position3D, limit: float) -> Position.Position3D:
    v_vec = v.vector
    norm = np.linalg.norm(v_vec)
    if norm > limit:
        return Position.Position3D(*[float(x) for x in v_vec * (limit / norm)])
    return v

def scale_position(pos: Position.Position3D, scalar: float) -> Position.Position3D:
    """Scale a position vector by a scalar."""
    return Position.Position3D(pos.x * scalar, pos.y * scalar, pos.z * scalar)
...
class Vehicle3D(Vehicle2D):
    ...
    def update(self, dt: float, window_w: int, window_h: int) -> None:
        ...
        if self.action == 'seek':
            ...
            self._acceleration = self.flee() if distance_to_target < 300 else Position.Position3D(0.0, 0.0, 0.0)
        elif self.action == 'arrive':
            self._acceleration = self.arrive()
        else:
            self._acceleration = Position.Position3D(0.0, 0.0, 0.0)
        ...
        if self._velocity.distance_to(Position.Position3D(0, 0, 0)) > 1e-6:
